...
actor NoteVectorStore {
    ...
    func removeIndex(for documentID: String) {
        loadFromDiskIfNeeded()
        chunks.removeAll { $0.documentID == documentID }
        // `storageURL` is optional (nil when persistence is disabled, or
        // when Application Support is unavailable) — unwrap before use.
        if let url = storageURL(for: documentID) {
