...
enum StudyDecision {
    ...
    /// Accent colour key — orange for urgency, green for normal, muted for soft.
    ...
    var detailLine: String {
        switch self {
        case .memoryRisk(let doc, let n, _):
            return "\(n) card\(n == 1 ? "" : "s") · ~\(max(1, n * 20 / 60)) min · \(doc.title)"
        case .due(let doc, let n), .weak(let doc, let n), .missedWindow(let doc, let n, _):
            return "\(n) card\(n == 1 ? "" : "s") · ~\(max(1, n * 20 / 60)) min · \(doc.title)"
        case .fresh(let doc):
...
//   B — ReturnIntent points to deleted document → stale intent
//   C — ReturnIntent fires for doc X, SRS urgency is on doc Y → split attention
...
enum ConflictLog {
    ...
    static func record(active: [Document], intent: ReturnIntent?, now: Date) {
        ...
        // A: Intent is active but SRS disagrees
        ...
        if intentDoc == nil {
            ...
            '\(intent.documentTitle)' not found in active library. \
            Clearing.
            """)
        ...
        if let intentDoc {
            ...
            if let topDueDoc, topDueDoc.id != intentDoc.id, topDue > 0 {
                ...
                SRS wins if intent riskScore < 0.5.
                """)
            }
        }
        #endif
